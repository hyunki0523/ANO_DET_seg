...
from nnunetv2.training.loss.compound_losses import DC_and_CE_loss, DC_and_BCE_loss, DC_and_topk_loss, DC_and_Focal_loss, DC_and_FocalTversky_loss
...
class nnUNetTrainer_SC(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        self.print_to_log_file("\n#######################################################################\n"
                               ...
                               "#######################################################################\n",
                               also_print_to_console=True, add_timestamp=True)
    ...
    def _build_loss(self):
        if self.label_manager.has_regions:
            # DC_and_Focal_loss 클래스를 사용하여 손실을 결합
            loss = DC_and_Focal_loss(
                ...
                focal_loss_kwargs={}, # Focal Loss 파라미터 설정
                weight_focal=1, # Focal Loss 가중치
                weight_dice=1, # Dice Loss 가중치
                ignore_label=self.label_manager.ignore_label, # 무시할 라벨 설정
                dice_class=MemoryEfficientSoftDiceLoss
            )
        else:
            # 기존의 DC_and_CE_loss 클래스 사용
